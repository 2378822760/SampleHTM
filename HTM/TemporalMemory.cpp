...
TemporalMemory::TemporalMemory(const map<string, string>& config) {
    vector<UInt> columnDimensions = stov(config.at("columnDimensions"));
    UInt cellsPerColumn = stoul(config.at("cellsPerColumn"));
    UInt activationThreshold = stoul(config.at("activationThreshold"));
    UInt LearningThreshold = stoul(config.at("LearningThreshold"));
    Permanence initialPermanence = stof(config.at("initialPermanence"));
    Permanence connectedPermanence = stof(config.at("connectedPermanence"));
    Permanence permanenceIncrement = stof(config.at("permanenceIncrement"));
    Permanence permanenceDecrement = stof(config.at("permanenceDecrement"));
    Permanence predictedSegmentDecrement = stof(config.at("predictedSegmentDecrement"));
    initialize(columnDimensions, cellsPerColumn, activationThreshold,
        initialPermanence, connectedPermanence, LearningThreshold,
        permanenceIncrement, permanenceDecrement,
        predictedSegmentDecrement);
...
vector<CellIndex> TemporalMemory::compute(std::size_t activeColumnsSize, const UInt* activeColumns, bool learn) {
    ...
    size_t sz = activeCells.size();
    for (int i = 0; i < sz; ++i) if (activeCells[i]) ret.push_back(i);
    updateCells();
    return ret;
}

UInt TemporalMemory::numberOfCells()
{
    return numColumns_ * cellsPerColumn_;
}
